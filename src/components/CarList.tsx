...
export default function CarList({ onSelectCar, lang, limit, onViewMore }: CarListProps) {
  ...
  const handleWhatsAppDirect = (carName: string) => {
    ...
    const message = lang === 'EN'
      ? `Hello Rinjani Transport, I would like to book the ${carName} car rental in Lombok. Please inform me about date availability and rates.`
      : `Halo Rinjani Transport, saya ingin memesan unit rental mobil ${carName} di Lombok. Mohon informasi ketersediaan tanggal dan penawaran harganya.`;
  ...
  return (
    <section id="cars" className="py-20 bg-gray-50 overflow-hidden font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Cars Grid: 4 items cleanly arranged */}
        <div 
          ...
          {displayedCars.map((car, index) => (
            ...
            >
              ...
              <div className="relative overflow-hidden aspect-[4/3] bg-gray-100">
                ...
                <div className="absolute top-4 right-0 bg-[#2563eb] text-white px-3.5 py-1.5 rounded-l-full rounded-r-none font-display font-bold text-[11px] shadow-md">
              ...
              <div className="p-5 sm:p-6 flex flex-col flex-grow space-y-3.5 text-left">
                <div className="space-y-1">
                  ...
                  <h3 className="font-display font-bold text-lg text-gray-900 group-hover:text-luxury-gold transition-colors leading-snug">
                    {car.name}
                  </h3>
                  <p className="font-sans text-xs text-gray-500 line-clamp-3 leading-relaxed pt-1">
                    {lang === 'EN' ? (car.descriptionEn || car.description) : car.description}
                  </p>
                </div>

                {/* Specs Specifications List with icons */}
                <div className="flex flex-wrap gap-x-3 gap-y-2 text-gray-500 text-xs py-2 border-t border-b border-gray-50">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(lang === 'EN' && car.includeListEn ? car.includeListEn : car.includeList).map((inc, i) => (
                      <span 
                        key={i} 
                        className="font-sans font-semibold text-[10px] text-luxury-gold bg-amber-50/60 px-2 py-0.5 rounded-full border border-amber-200/40"
                      >
                        {inc}
                      </span>
                    ))}
                ...
                {/* Bottom Actions */}
                <div className="pt-3 mt-auto border-t border-gray-100 flex items-center gap-2.5">
                  <button
                    onClick={() => onSelectCar(car)}
                    className="flex-grow bg-[#2563eb] hover:bg-blue-700 text-white font-display font-bold text-xs py-3 px-3 rounded-xl shadow-md transition-all flex items-center justify-center gap-1.5 cursor-pointer"
